  ...
  if self:GetIsAlive() and self:GetCanDie() then
         ...
         if self:isa("Alien") then
          ...
          if GetHasRebirthUpgrade(self) and self:GetEligableForRebirth() then
            ...
            if Server and GetConductor():GetIsPhaseTwoBoolean() and math.random(100) <= 30 then  
             if  self:isa("Skulk")  then
              // CreateEntity(Rupture.kMapName, point, 2) //ERror here because owner is destroyed
              local rupt = CreateEntity(Rupture.kMapName, point, 2) //hm?
              rupt:SetOwner(self)
