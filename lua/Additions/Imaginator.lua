...
local function GetHasFourChairs()
local CommandStations = #GetEntitiesForTeam( "CommandStation", 1 )

if CommandStations >= 4 then return true end
...
function Imaginator:SpawnRandomChair(powerpoint)
               ...
               CreateEntityForTeam(kTechId.CommandStation, randomspawn, 1)
...
local powerpoint = GetRandomActivePower()
   if not GetHasFourChairs() and not GetHasCCInRoom(powerpoint:GetOrigin()) then
      self:SpawnRandomChair(powerpoint)
      return 
   end

local chair = GetRandomCC()
local success = false
local entity = nil
            if chair and tospawn then
                 local potential = FindPosition(GetAllLocationsWithSameName(chair:GetOrigin()), chair, 1)
