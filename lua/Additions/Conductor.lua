...
local function BuildDisabledPower(player, where)
 ...
 local function ForAllMarinesDefendArc()
          for _, player in ientitylist(Shared.GetEntitiesWithClassname("Marine")) do //if not virtual then
               if player:GetIsAlive() and not player:isa("Commander") then
                  local nearestARC =  GetNearest(player:GetOrigin(), "ARC", nil, function(ent) return ent.mode == ARC.kMode.Moving   end) 
                   if not nearestARC then break end
                   player:GiveOrder(kTechId.Defend, nearestARC:GetId(), nearestARC:GetOrigin(), nil, true, true)
                end
         end
end
