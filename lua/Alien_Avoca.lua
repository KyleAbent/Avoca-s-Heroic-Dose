...
function Alien:ChangeClass()
        self:AddTimedCallback(function() self:DoChangeClass() end, 0.6) 
end

function Alien:DoChangeClass()
if Server then
   local spawnOrigin = self:GetOrigin()
   
   // if math.random(1,3) == 3 then
       local toSpawn = math.random(1,4)
       local team = GetGamerules():GetTeam(2)
       
       if toSpawn == 1 then
             team:ReplaceRespawnPlayer(self, spawnOrigin, self:GetAngles(), Gorge.kMapName)   
       elseif toSpawn == 2 then
               team:ReplaceRespawnPlayer(self, spawnOrigin, self:GetAngles(), Lerk.kMapName)   
       elseif toSpawn == 3 then
              team:ReplaceRespawnPlayer(self, spawnOrigin, self:GetAngles(), Fade.kMapName)   
       elseif toSpawn == 4 and  GetConductor():GetIsPhaseTwo() then
             team:ReplaceRespawnPlayer(self, spawnOrigin, self:GetAngles(), Onos.kMapName) 
       end
       
                  if self.lastUpgradeList then
                    self.upgrade1 = self.lastUpgradeList[1] or 1
                    self.upgrade2 = self.lastUpgradeList[2] or 1
                    self.upgrade3 = self.lastUpgradeList[3] or 1
                   end
