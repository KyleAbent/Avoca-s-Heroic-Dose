...
local umm = client
          ...
          local team = math.random(1,2)
                    Shared.ConsoleCommand(string.format("sh_setteam %s %s", client:GetUserId(), team )) 
                    Shared.ConsoleCommand(string.format("sh_gag %s 9001", client:GetUserId() ))
                    
                if not GetGamerules():GetGameStarted() then
                self:SimpleTimer( 4, function() 
                GetGamerules():SetMaxBots(12, false)
                                Shared.ConsoleCommand(string.format("sh_direct %s", umm:GetUserId() ))
                                 Shared.ConsoleCommand(string.format("addbot" ))
                                  Shared.ConsoleCommand(string.format("sh_randomrr" ))
                    //    for e = 1, 12 do
                    //   Shared.ConsoleCommand(string.format("addbot" ))
                     //    end
...
function Plugin:ClientConfirmConnect(client)
     ...
     local team = math.random(1,2)
   --  self:SimpleTimer( 4, function() 
     if client and not client:GetUserId() == 22542592 then
